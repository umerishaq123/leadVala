...
class NotificationProvider with ChangeNotifier {
  ...
  getNotificationList(context) async {
    ...
    try {
      ...
      print("🔹 API Request to: ${api.notifications}");
      ...
      print("🔹 Raw Response: ${json.encode(response.data)}");

      if (response.statusCode == 200) {
        if (response.data is Map<String, dynamic>) {
          // ✅ Extract the "data" list if the response is a Map
          if (response.data.containsKey("data") &&
              response.data["data"] is List) {
            List notifications = response.data["data"];
          ...
          // ✅ Directly use the response if it's already a list
          notificationList =
        ...
        print("✅ Notifications Loaded: ${notificationList.length}");
        notifyListeners();
      } else {
        print("❌ API Error: ${response.statusMessage}");
      }
    } catch (e) {
      log("❌ ERROR: getNotificationList failed: $e");
      notifyListeners();
    }
  }
  ...
  getNotificationList(context) async {
    ...
    try {
      await apiServices
          ...
          .then((value) {
        print('api notificationss${value.data}');
        if (value.isSuccess!) {
          List address = value.data;
          notificationList = [];
          print('hellow notfication${notificationList}');
...
*/
  ...
  onTap(NotificationModel model, context) {
    ...
    if (model.data!.type == "provider") {
      ...
      print('showing json datass ${model.data!.toJson()}');
